...
const NAV_ITEMS: NavItem[] = [
  ...
  {
    href: "/recepcao/novo-atendimento",
...
export function Sidebar({ user }: { user: Profile }) {
  ...
  const items = NAV_ITEMS.filter((item) =>
    item.roles.includes(user.role_key)
  );
    ...
    <aside className="w-[232px] shrink-0 bg-primary-dark text-white flex flex-col p-3.5">
      ...
      <div className="flex items-center gap-2.5 px-2 pb-5">
        ...
        <div className="leading-tight">
          ...
          <span className="block text-[11px] text-[#BFE0CD]">
            São José de Ribamar
          </span>
        </div>
      </div>
      ...
      <nav className="flex flex-col gap-0.5 flex-1">
        ...
        {/* Painel público */}
        <div className="text-[10.5px] uppercase tracking-wide text-[#7FAF97] px-2.5 pt-3.5 pb-1">
          Painel
        </div>
        ...
        <Link
          href="/painel"
          target="_blank"
          className="flex items-center gap-2.5 px-2.5 py-2.5 rounded-lg text-[13.5px] font-medium text-[#D7ECE0] hover:bg-white/10"
        >
          <Tv size={16} />
          Painel público (TV)
        </Link>
      ...
      <div className="pt-3 border-t border-white/10 mt-2">
        <div className="flex items-center gap-2.5 p-2 rounded-lg">
          ...
          <div className="leading-tight flex-1 min-w-0">
            ...
            <span className="text-[11px] text-[#9FCBB1]">
              {ROLE_LABEL[user.role_key]}
            </span>
          </div>
        </div>
        ...
        <form action={logout}>
          <button
            type="submit"
            className="flex items-center gap-1.5 text-[12px] text-[#9FCBB1] hover:text-white px-2 py-1.5 rounded-md mt-1 w-full text-left"
          >
            <LogOut size={13} />
            Sair
          </button>
