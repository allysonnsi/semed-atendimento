...
// descrevem — geração de sequência, update condicional em "callNext",
// justificativa obrigatória para prioridade e log de auditoria.
//
// IMPORTANTE:
// A autenticação do sistema já utiliza o Supabase Auth.
// Esta camada ainda é MOCK para os dados operacionais.
// ============================================================================
...
function seed(): Store {
  ...
  const profiles: Profile[] = [
    ...
    {
      id: "u-att-rh",
      full_name: "Marina Alves Costa",
      role_id: "role-attendant",
      role_key: "attendant",
      sector_id: "sec-rh",
      active: true,
      created_at: now,
  ...
  const visitorTypes: VisitorType[] = [
    ...
    id: "vt-" + label.toLowerCase().replace(/[^a-z]+/g, "-"),
    label,
    active: true,
  }));
  ...
  for (let i = 0; i < 16; i++) {
    ...
    tickets.push({
      ...
      priority_justification:
        priority !== "normal"
          ? "Idoso / situação de saúde relatada"
          : null,
      status,
      created_by: "u-recep",
...
export function listTicketsToday(): Ticket[] {
  ...
  return getStore().tickets.filter(
    (t) => t.ticket_date === today
  );
}

export function listAllTickets(): Ticket[] {
  return [...getStore().tickets].sort((a, b) =>
...
export function getAttendanceForTicket(
  ticketId: string
): Attendance | undefined {
  return getStore().attendances.find(
...
export function createTicket(
  ...
  const sector = store.sectors.find(
    (s) => s.id === input.sectorId
  );

  if (!sector || !sector.active) {
    throw new Error(
      "Setor inválido ou inativo."
    );
  }

  if (
    input.priority !== "normal" &&
    !input.priorityJustification
  ) {
    throw new Error(
      "Justificativa obrigatória para atendimento com prioridade."
    );
  }

  const vt = store.visitorTypes.find(
    (v) => v.id === input.visitorTypeId
  );
  ...
  const existing = store.tickets.filter(
    (t) =>
  ...
  const ticket: Ticket = {
    ...
    priority_justification:
      input.priorityJustification || null,
    status: "waiting",
...
export function recallTicket(
  ...
  const att = store.attendances.find(
    (a) => a.ticket_id === ticketId
  );
...
export function addUser(
  ...
  // Atendente obrigatoriamente precisa de setor.
  if (
    roleKey === "attendant" &&
    !sectorId
  ) {
    throw new Error(
      "Atendente deve possuir um setor."
    );
  }

  // Admin e recepcionista não possuem setor.
